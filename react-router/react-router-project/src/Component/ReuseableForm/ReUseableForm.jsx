...
const ReUseableForm = ({formTitle,btnText}) => {
    const handleFormSubmit = (e) => {
        e.preventDefault();
    }
    return (
        <div>
            <h1 className="text-5xl mt-10">{formTitle}</h1>
            <div className="border-2 rounded-lg py-3">
                <form onSubmit={handleFormSubmit}>
                    ...
                    <input className="py-2 px-3 my-2 rounded-lg bg-gray-900 cursor-pointer" type="submit" value={btnText} />
                </form>
            </div>
        </div>
    );
